...
void check_for_existence(Ident &ident, VarEnv *env_of_vars, int &line_number, Visitable* v) {
  if(env_of_vars->find(ident) == env_of_vars->end())
    fail("LINE " + to_string(line_number) + " : " + ident + " is not defined\n", v);
}

void check_for_existence(Ident &ident, FunEnv *env_of_functions, int &line_number, Visitable* v) {
  if(env_of_functions->find(ident) == env_of_functions->end())
    fail("LINE " + to_string(line_number) + " : " + ident + " is not defined\n", v);
}

void check_for_type_matching(TYPE type1, TYPE type2, int &line_number, Visitable *v) {
  if(type1 != type2) 
    fail("LINE " + to_string(line_number) + " : trying to assign " + type2 + " to " + type1 + "\n", v);
}

void same_scope_redefinition(Ident ident, std:: set<Ident> *defined_in_current_scope, int line_number, Visitable* v) {
  if(defined_in_current_scope->find(ident) != defined_in_current_scope->end())
    fail("LINE " + to_string(line_number) + " : variable " + ident + " got redefined in this same scope\n", v);
}
...
void StaticAnalyzer::visitFnDef(FnDef *p)
{
  ...
  if(last_return != get_type(p->type_)) 
    fail("LINE " + to_string(p->line_number) + " : function returned " + last_return + " but should return " + get_type(p->type_) + "\n", p);
...
void StaticAnalyzer::visitStructDef(StructDef *p) {
  if(p->ident_ == this->INT || p->ident_ == this->VOID || p->ident_ == this->BOOLEAN || p->ident_ == this->STRING) {
    fail("LINE " + to_string(p->line_number) + " : you can't have " + p->ident_ + " as a struct name\n", p);
  }
  ...
  for(int i = 0; i < p->liststructmember_->size(); i++) {
    ...
    if(auto init_member = dynamic_cast<StructMemNoInit*>(member)) 
      if(!is_a_valid_type(init_member->type_, env_of_structures)) 
        fail("LINE " + to_string(init_member->line_number) + " : " + get_type(init_member->type_) + " does not name a valid type\n", init_member);
    ...
    if(auto init_member = dynamic_cast<StructMemInit*>(member)) {
      ...
      check_for_type_matching(get_type(init_member->type_), last_evaluated_expr, init_member->line_number, p);
    }
  }
}
...
void StaticAnalyzer::visitIdentExp(IdentExp *p) {
  ...
  if(env_of_structures.find(last_evaluated_expr) == env_of_structures.end()) 
    fail("LINE " + to_string(p->line_number) + " : " + last_evaluated_expr + " does not name a valid type\n", p);
  
  if(env_of_structures[last_evaluated_expr].find(p->ident_) == env_of_structures[last_evaluated_expr].end()) 
    fail("LINE " + to_string(p->line_number) + " : struct does not have " + p->ident_ + " as a member\n", p);
...
void StaticAnalyzer::visitIdentExpSimple(IdentExpSimple *p) {
  check_for_existence(p->ident_, &env_of_vars, p->line_number, p);
      ...
      for(int j = 0 ; j < fn_def->listarg_->size(); j++) {
        if(auto arg = dynamic_cast<ArgDef*>((*(fn_def->listarg_))[j])) {
          if(get_type(arg->type_) == this->VOID)
            fail("LINE " + to_string(fn_def->line_number) + " : function can't have void as an argument\n", arg);
          arg_types.push_back(get_type(arg->type_));
        }
      }
      if(env_of_functions.find(fn_def->ident_) != env_of_functions.end())
        fail("LINE " + to_string(fn_def->line_number) + " : function with name " + fn_def->ident_ + " already exists\n", fn_def);
      ...
      for(auto item : *items) {
        if(auto member = dynamic_cast<StructMemNoInit*>(item)) {
          if(get_type(member->type_) == this->VOID) 
            fail("LINE " + to_string(struct_def->line_number) + " : struct can't have VOID as an attribute\n", member);
        ...
        if(auto member = dynamic_cast<StructMemInit*>(item)) {
          ...
          check_for_type_matching(last_evaluated_expr, get_type(member->type_), member->line_number, member);
          members[member->ident_] = get_type(member->type_);
        }
      }
      
      if(env_of_structures.find(struct_def->ident_) != env_of_structures.end())
        fail("LINE " + to_string(struct_def->line_number) + " : struct was defined before\n", struct_def);
      env_of_structures[struct_def->ident_] = members;
    }
  }
  
  if(!was_there_main) {
    cerr << "No function main present\n";
    exit(-1);
  }
...
void StaticAnalyzer::visitDecl(Decl *p)
{
  if(get_type(p->type_) == this->VOID) 
    fail("LINE " + to_string(p->line_number) + " : can't have void as a variable type\n", p);

  if(!is_a_valid_type(p->type_, env_of_structures))
    fail("LINE " + to_string(p->line_number) + " : " + get_type(p->type_) + " was not recognized as a valid type\n", p);


  if(p->listitem_) {
    for (ListItem::const_iterator i = p->listitem_->begin() ; i != p->listitem_->end() ; ++i) {
      if(auto init = dynamic_cast<NoInit*>(*i)) {
        same_scope_redefinition(init->ident_, &defined_in_current_scope, init->line_number, init);
      ...
      if(auto init = dynamic_cast<Init*>(*i)) {
        ...
        check_for_type_matching(get_type(p->type_), last_evaluated_expr, init->line_number, init);
        defined_in_current_scope.insert(init->ident_);
        env_of_vars[init->ident_] = get_type(p->type_);
      }
      if(auto init = dynamic_cast<InitStruct*>(*i)) {
        same_scope_redefinition(init->ident_1, &defined_in_current_scope, init->line_number, init);
...
void StaticAnalyzer::visitNewStruct(NewStruct *p) {
  ...
  p->identexpan_->accept(this);
  check_for_type_matching(p->ident_, last_evaluated_expr, p->line_number, p);
}
...
void StaticAnalyzer::visitAss(Ass *p)
{
  ...
  p->expr_->accept(this);
  check_for_type_matching(var_type, last_evaluated_expr, p->line_number, p);
}

void StaticAnalyzer::visitIncr(Incr *p)
{
  p->identexpan_->accept(this);
  check_for_type_matching(last_evaluated_expr, this->INT, p->line_number, p);
}
...
void StaticAnalyzer::visitRet(Ret *p)
{
  ...
  if(expected_return_type != last_evaluated_expr) 
    fail("LINE " + to_string(p->line_number) + " : wanted to return " + last_evaluated_expr + " but expected " + expected_return_type + "\n", p);

  if(last_evaluated_expr == this->VOID)
    fail("LINE " + to_string(p->line_number) + " : you can't return void that way\n", p);
...
void StaticAnalyzer::visitVRet(VRet *p)
{
  ...
  if(last_return != expected_return_type) 
    fail("LINE " + to_string(p->line_number) + " : function returns " + last_return + " but it expects " + expected_return_type + " type\n", p);
}
...
void StaticAnalyzer::visitCond(Cond *p)
{
  ...
  if(last_evaluated_expr != this->BOOLEAN) 
    fail("LINE " + to_string(p->line_number) + " : if must have condition of type boolean\n", p);
...
void StaticAnalyzer::visitCondElse(CondElse *p)
{
  p->expr_->accept(this);
  if(last_evaluated_expr != this->BOOLEAN)
    fail("LINE " + to_string(p->line_number) + " : if else must have condition of type boolean\n", p);
...
void StaticAnalyzer::visitWhile(While *p)
{
  p->expr_->accept(this);
  if(last_evaluated_expr != this->BOOLEAN) 
    fail("LINE " + to_string(p->line_number) + " : while must have condition of type boolean\n", p);
...
{
  check_for_existence(p->ident_, &env_of_functions, p->line_number, p);
  auto expected_arg_types = env_of_functions[p->ident_].first;
  if(p->listexpr_->size() != expected_arg_types.size()) 
    fail("LINE " + to_string(p->line_number) + " : function " + p->ident_ + " was supplied " + to_string(p->listexpr_->size()) + " arguments but needed " + to_string(expected_arg_types.size()) + "\n", p);

  for(int i = 0; i < p->listexpr_->size(); i++) {
    (*(p->listexpr_))[i]->accept(this);
    check_for_type_matching(expected_arg_types[i], last_evaluated_expr, p->line_number, p);
  }
...
{
  p->expr_->accept(this);
  check_for_type_matching(last_evaluated_expr, this->BOOLEAN, p->line_number, p);
...
void StaticAnalyzer::visitEAdd(EAdd *p)
{
  ...
  if(exp1 != exp2) 
    fail("LINE " + to_string(p->line_number) + " : you can only add/substract expressions of the same type.\n", p);
  ...
  if(auto val1 = std::get_if<std::string>(&v1)) 
    if(auto val2 = std::get_if<std::string>(&v2)) {
      if(dynamic_cast<Plus*>(p->addop_))
        ...
        fail("LINE " + to_string(p->line_number) + " : you can't substract strings.\n", p); 
    }
...
void StaticAnalyzer::visitERel(ERel *p)
{
  ...
  if(type1 != type2 || type1 == this->VOID || type2 == this->VOID) 
    fail("LINE " + to_string(p->line_number) + " : cannot compare " + type1 + " and " + type2 + "\n", p);

  if(type1 == this->BOOLEAN) {
    if(!(dynamic_cast<EQU*>(p->relop_) || dynamic_cast<NE*>(p->relop_))) {
      fail("LINE " + to_string(p->line_number) + " : this operation is not allowed for " + type1 + "\n", p);
      exit(-1);
    }
  ...
  if(!(dynamic_cast<EQU*>(p->relop_) || dynamic_cast<NE*>(p->relop_))) 
    fail("LINE " + to_string(p->line_number) + " : you can only == and != for " + type1 + "\n", p);
}
